        ...
        <div className="p-4 pt-6">
          ...
          <div className="mb-6">
            ...
            <nav className="space-y-1">
              <SideNavItem icon={<Stethoscope size={18} />} label="Command Center" to="/clinical/command-center" />
              <SideNavItem icon={<Calendar size={18} />} label="Appointments" to="/clinical/appointments" />
            </nav>
          </div>
          ...
          <div className="mb-6">
            ...
            <nav className="space-y-1">
              <SideNavItem icon={<ClipboardPlus size={18} />} label="Triage Pre-check" to="/operations/nurse-triage" />
            </nav>
          </div>
          ...
          <div className="mb-6">
            ...
            <nav className="space-y-1">
              <SideNavItem icon={<LayoutDashboard size={18} />} label="Operations Dashboard" to="/admin/dashboard" />
            </nav>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-3 px-3 py-1.5 bg-surface-container rounded-full mt-4 mx-2">
            <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold">
              {user?.charAt(0)?.toUpperCase() || 'D'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-on-surface truncate">{user || 'Clinician'}</p>
              <p className="text-[10px] uppercase tracking-wider text-on-surface-variant font-semibold">Ready</p>
            </div>
          </div>
        </div>
              ...
              <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-outline rounded-xl shadow-2xl overflow-hidden z-20">
                ...
                <div className="max-h-60 overflow-y-auto">
                  {isSearchLoading ? (
                    ...
                    <div className="p-4 text-center text-xs text-on-surface-variant italic">No matches found in clinical registry.</div>
                  ) : (
